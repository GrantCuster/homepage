import { r as requireReact, a as requireReactDom, g as getDefaultExportFromCjs } from "./index-TS6MFGZj.js";
import require$$0 from "util";
var reactExports = requireReact();
var client_node_unbundled = { exports: {} };
var reactServerDomWebpackClient_node_unbundled_production = {};
/**
 * @license React
 ...
 */
var hasRequiredReactServerDomWebpackClient_node_unbundled_production;
function requireReactServerDomWebpackClient_node_unbundled_production() {
  if (hasRequiredReactServerDomWebpackClient_node_unbundled_production) return reactServerDomWebpackClient_node_unbundled_production;
  hasRequiredReactServerDomWebpackClient_node_unbundled_production = 1;
  var util = require$$0, ReactDOM = requireReactDom(), decoderOptions = { stream: true };
  ...
  function processReply(root, formFieldPrefix, temporaryReferences, resolve, reject) {
    ...
    function resolveToJSON(key, value) {
      ...
      if ("object" === typeof value) {
        ...
        if (void 0 !== parentReference)
          ...
          -1 === key.indexOf(":") && (parentReference = writtenObjects.get(this), void 0 !== parentReference && (key = parentReference + ":" + key, writtenObjects.set(value, key), void 0 !== temporaryReferences));
    ...
    function serializeModel(model, id) {
      "object" === typeof model && null !== model && (id = "$" + id.toString(16), writtenObjects.set(model, id), void 0 !== temporaryReferences);
      modelRoot = model;
  ...
  function processFullStringRow(response, id, tag, row) {
    switch (tag) {
      ...
      case 72:
        ...
        switch (id) {
          ...
          case "X":
            ...
            break;
          case "S":
            "string" === typeof response ? row.S(response) : row.S(
              response[0],
      ...
      case 69:
        ...
        chunk ? triggerErrorOnChunk(chunk, row) : tag.set(id, new ReactPromise("rejected", null, row, response));
        break;
      case 84:
        tag = response._chunks;
        (chunk = tag.get(id)) && "pending" !== chunk.status ? chunk.reason.enqueueValue(row) : tag.set(id, new ReactPromise("fulfilled", row, null, response));
        break;
      case 68:
      case 87:
        throw Error(
      ...
      case 67:
        ...
        break;
      default:
        tag = response._chunks, (chunk = tag.get(id)) ? resolveModelChunk(chunk, row) : tag.set(
          id,
  ...
  reactServerDomWebpackClient_node_unbundled_production.createFromNodeStream = function(stream, serverConsumerManifest, options) {
  ...
  reactServerDomWebpackClient_node_unbundled_production.createServerReference = function(id) {
    return createServerReference$1(id, noServerCall);
  };
  return reactServerDomWebpackClient_node_unbundled_production;
}
var hasRequiredClient_node_unbundled;
function requireClient_node_unbundled() {
  if (hasRequiredClient_node_unbundled) return client_node_unbundled.exports;
  hasRequiredClient_node_unbundled = 1;
  {
    client_node_unbundled.exports = requireReactServerDomWebpackClient_node_unbundled_production();
  }
  return client_node_unbundled.exports;
}
var client_node_unbundledExports = requireClient_node_unbundled();
const RSDWClient = /* @__PURE__ */ getDefaultExportFromCjs(client_node_unbundledExports);
const isErrorInfo = (x) => {
...
const erroredElementsPromiseMap = /* @__PURE__ */ new WeakMap();
...
const unstable_callServerRsc = async (funcId, args, fetchCache = defaultFetchCache) => {
  ...
  const createData = (responsePromise2) => createFromFetch(checkStatus(responsePromise2), {
    callServer: (funcId2, args2) => unstable_callServerRsc(funcId2, args2, fetchCache)
  });
...
const prefetchedParams = /* @__PURE__ */ new WeakMap();
const fetchRscInternal = (url, rscParams, fetchCache) => {
  const enhanceFetch = fetchCache[ENHANCE_FETCH] || ((f) => f);
  return rscParams === void 0 ? enhanceFetch(fetch)(url) : rscParams instanceof URLSearchParams ? enhanceFetch(fetch)(url + "?" + rscParams) : encodeReply(rscParams).then((body) => enhanceFetch(fetch)(url, {
    method: "POST",
    body
  }));
};
...
const Root = ({ initialRscPath, initialRscParams, fetchCache = defaultFetchCache, unstable_enhanceFetch, unstable_enhanceCreateData, children }) => {
  ...
  const refetch = reactExports.useCallback((rscPath, rscParams) => {
  ...
  return reactExports.createElement(RefetchContext.Provider, {
    value: refetch
  }, reactExports.createElement(ElementsContext.Provider, {
    value: elements
...
const ChildrenContextProvider = reactExports.memo(ChildrenContext.Provider);
const ErrorContext = reactExports.createContext(void 0);
const ErrorContextProvider = reactExports.memo(ErrorContext.Provider);
const Children = () => reactExports.use(ChildrenContext);
const ThrowError_UNSTABLE = () => {
  const errAndReset = reactExports.use(ErrorContext);
  if (errAndReset) {
    throw errAndReset[0];
  }
  return null;
};
const useResetError_UNSTABLE = () => {
  const errAndReset = reactExports.use(ErrorContext);
  if (errAndReset) {
...
class GeneralErrorHandler extends reactExports.Component {
  constructor(props) {
    super(props);
    this.state = {
      error: null
  ...
  render() {
    ...
    if (error !== null) {
      if (this.props.errorHandler) {
        return reactExports.createElement(ErrorContextProvider, {
          value: [
            error,
            this.reset
          ]
...
const Slot = ({ id, children, unstable_handleError, unstable_fallback }) => {
  const [errorHandler, setErrorHandler] = reactExports.useState();
  const setValidElement = reactExports.useCallback((element) => setErrorHandler(reactExports.createElement(ChildrenContextProvider, {
    value: unstable_handleError
  }, element)), [
    unstable_handleError
  ]);
  if (unstable_handleError !== void 0) {
    return reactExports.createElement(GeneralErrorHandler, {
      errorHandler
    }, reactExports.createElement(InnerSlot, {
      id,
      setValidElement
    }, children));
  }
  return reactExports.createElement(InnerSlot, {
    id,
    unstable_fallback
  }, children);
};
const INTERNAL_ServerRoot = ({ elementsPromise, children }) => reactExports.createElement(ElementsContext.Provider, {
